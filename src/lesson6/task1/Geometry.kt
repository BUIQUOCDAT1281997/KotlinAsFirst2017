...
data class Circle(val center: Point, val radius: Double) {
     ...
     */
    fun contains(p: Point): Boolean {
        return Math.sqrt(sqr(p.x - center.x) + sqr(p.y - center.y)) <= radius
    }
}
...
class Line private constructor(val b: Double, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        val x = (other.b / Math.cos(other.angle) - b / Math.cos(angle)) / (Math.tan(angle) - Math.tan(other.angle))
...
fun lineBySegment(s: Segment): Line {
    val corner = Math.atan((s.begin.y - s.end.y) / (s.begin.x - s.end.x))
    if (corner < 0.0) return Line(s.begin, Math.PI + corner)
    else return Line(s.begin, corner)
}
...
fun bisectorByPoints(a: Point, b: Point): Line {
    ...
    var angle = 0.0
    if (line.angle < Math.PI / 2) angle = line.angle + Math.PI / 2
    else angle = line.angle - Math.PI / 2
...
fun circleByThreePoints(a: Point, b: Point, c: Point): Circle {
    ...
    return Circle(x.crossPoint(y), x.crossPoint(y).distance(a))
}
